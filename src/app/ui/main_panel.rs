...
impl WeakAuraImporter {
    pub(crate) fn render_main_content(&self) -> Element<'_, Message> {
        ...
        // Step 2 Header
        content = content.push(text("Step 2: Load WeakAuras").size(16).color(colors::GOLD));

        // Action Buttons row
        let paste_btn = if self.show_paste_input {
            button(text("Paste").color(colors::BG_DARKEST))
        ...
        let load_file_btn = if self.is_loading {
            button(text("Load file")).style(theme::button_secondary)
        } else {
        ...
        let load_folder_btn = if self.is_loading {
            button(text("Load folder")).style(theme::button_secondary)
        } else {
            button(text("Load folder"))
                .style(theme::button_secondary)
        ...
        if !self.parsed_auras.is_empty() {
            content = content.push(space::vertical().height(Length::Fixed(16.0)));
            content = content.push(
                container(text(""))
    ...
    fn render_paste_input_area(&self) -> Element<'_, Message> {
        let mut paste_content = Column::new().spacing(8);
        ...
        let paste_clipboard_btn = button(text("Paste from clipboard"))
            .style(theme::button_secondary)
            .on_press(Message::PasteFromClipboard);

        let parse_btn = button(text("Parse").color(colors::BG_DARKEST))
            .style(theme::button_primary)
    ...
    fn render_review_import_section(&self) -> Element<'_, Message> {
        ...
        // Header
        content = content.push(text("Step 3: Review & Import").size(16).color(colors::GOLD));
        ...
        let select_all_btn = if self.is_importing {
            button(text("Select All")).style(theme::button_secondary)
        } else {
        ...
        let deselect_all_btn = if self.is_importing {
            button(text("Deselect All")).style(theme::button_secondary)
        } else {
        ...
        let remove_selected_btn = if has_selected && !self.is_importing && !self.is_loading {
            ...
            button(text("Remove Selected")).style(theme::button_secondary)
        };

        // Import button
        let import_btn = if can_import {
            button(text("Import Selected >>").color(colors::BG_DARKEST))
                .style(theme::button_primary)
                .on_press(Message::ShowImportConfirm)
        } else {
            button(text("Import Selected >>").color(colors::TEXT_MUTED))
                .style(theme::button_secondary)
        };
        ...
        let controls_row = if !can_import && self.selected_sv_path.is_none() && !self.is_importing {
            row![
                ...
                button(text("Import Selected >>").color(colors::TEXT_MUTED))
                    .style(theme::button_secondary),
                text("Select a SavedVariables file first").color(colors::TEXT_MUTED),
                space::horizontal(),
                text(stats_format).color(colors::TEXT_SECONDARY)
            ]
            .spacing(8)
            .align_y(iced::Alignment::Center)
        } else {
    ...
    fn render_aura_list(&self) -> Element<'_, Message> {
        ...
        for (idx, entry) in self.parsed_auras.iter().enumerate() {
            ...
            // Remove button
            let remove_btn = button(text("X").color(colors::ERROR).size(12))
            ...
            let label_btn = button(text(summary).color(label_color))
            ...
            let mut item_row = row![remove_btn].spacing(8).align_y(iced::Alignment::Center);
        ...
        let list_container = container(scrollable(list_col).height(Length::Fill))
            .style(theme::container_elevated)
            .padding(8)
            .height(Length::Fill);
