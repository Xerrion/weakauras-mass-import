...
mod tests {
    ...
    fn test_serialize_mixed_table() {
        // Create a mixed table like triggers: array part + hash part
        ...
        // Should NOT contain ["1"] - array indices should be implicit
        assert!(
            !serialized.contains("[\"1\"]"),
            "Mixed table should not have explicit [\"1\"] key"
        );

        // Should contain the hash keys with explicit brackets
        assert!(
        ...
        // Should have comment showing implicit index
        assert!(
            serialized.contains("-- [1]"),
    ...
    fn test_parse_mixed_table() {
        // Parse a table that has both implicit array entries and named keys
